...
async function processFiles({
  ...
  if (storageType === StorageType.ArweaveSafe) {
    //@ts-ignore
...
export async function* makeArweaveBundleUploadGenerator(
  ...
  if (storageType === StorageType.ArweaveSafe && !walletKeyPair) {
    throw new Error(
  ...
  const bundlr =
    storageType === StorageType.ArweaveSafe
      ? env === 'mainnet-beta'
        ? new Bundlr(
  ...
  while (filePairs.length) {
    const { count, size } = await getBundleRange(
      filePairs,
      storage === StorageType.ArweaveSafe,
    );
