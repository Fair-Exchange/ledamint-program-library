...
async fn buy_success() {
    ...
    airdrop(&mut context, &test_metadata.token.pubkey(), ONE_SAFE)
        .await
        .unwrap();
    test_metadata
        .create(
    ...
    airdrop(&mut context, &buyer.pubkey(), ONE_SAFE * 10)
        .await
        .unwrap();
    let (_, deposit_tx) = deposit(&mut context, &ahkey, &ah, &test_metadata, &buyer, ONE_SAFE);
    context
        .banks_client
    ...
    assert_eq!(bid_receipt.price, ONE_SAFE);
