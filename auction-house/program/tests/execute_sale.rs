...
use safecoin_sdk::{commitment_config::CommitmentLevel, signer::Signer};
...
async fn auctioneer_execute_sale_success() {
    ...
    airdrop(&mut context, &auctioneer_authority.pubkey(), ONE_SAFE)
        .await
        .unwrap();
...
async fn auctioneer_execute_public_sale_success() {
    ...
    // Delegate external auctioneer authority.
    let auctioneer_authority = Keypair::new();
    airdrop(&mut context, &auctioneer_authority.pubkey(), ONE_SAFE * 2)
        .await
        .unwrap();
    ...
    airdrop(&mut context, &ah.auction_house_fee_account, ONE_SAFE * 10)
        .await
        .unwrap();
