    ...
    match SplAccount::unpack(&src_account.data.borrow()) {
        ...
        Err(_) => {
            if mint.key() != safe_token::native_mint::id() {
                return err!(AuctionHouseError::ExpectedSafeAccount);
            }

            if !src_wallet.is_signer {
                return err!(AuctionHouseError::SAFEWalletMustSign);
            }
...
/// <https://github.com/fair-exchange/safecoin-program-library/blob/7d4873c61721aca25464d42cc5ef651a7923ca79/associated-token-account/program/src/processor.rs#L51-L98>
#[inline(always)]
