...
//! AuctionHouse is a protocol for marketplaces to implement a decentralized sales contract. It is simple, fast and very cheap. AuctionHouse is a Safecoin program available on Mainnet Beta and Devnet. Anyone can create an AuctionHouse and accept any SPL token they wish.
//!
...
pub mod auction_house {
    ...
    /// Create a private buy bid by creating a `buyer_trade_state` account and an `escrow_payment` account and funding the escrow with the necessary SAFE or SPL token amount.
    pub fn buy<'info>(
    ...
    /// Create a public buy bid by creating a `public_buyer_trade_state` account and an `escrow_payment` account and funding the escrow with the necessary SAFE or SPL token amount.
...
pub struct CreateAuctionHouse<'info> {
    /// Treasury mint account, either native SAFE mint or a SPL token mint.
    pub treasury_mint: Account<'info, Mint>,

    /// Key paying SAFE fees for setting up the Auction House.
    #[account(mut)]
    ...
    /// CHECK: User can use whatever they want for intialization.
    /// SAFE or SPL token account to receive Auction House fees. If treasury mint is native this will be the same as the `treasury_withdrawl_destination_owner`.
    #[account(mut)]
