...
mod pnft {
    ...
    async fn delegate_burn_token_account_must_match_mint() {
        ...
        let init_token_ix = safe_token::instruction::initialize_account(
            &safe_token::ID,
            &new_wallet_token.pubkey(),
            &da.mint.pubkey(),
            &new_wallet.pubkey(),
...
mod fungible {
    ...
    async fn owner_burn() {
        ...
        let update_authority = context.payer.dirty_clone();
        let owner = Keypair::new();
        owner.airdrop(&mut context, LAMPORTS_PER_SAFE).await.unwrap();
    ...
    async fn only_owner_can_burn() {
        ...
        not_owner
            .airdrop(&mut context, LAMPORTS_PER_SAFE)
            .await
            .unwrap();
