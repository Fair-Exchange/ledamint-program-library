...
async fn deposit_success() {
    ...
    airdrop(&mut context, &buyer.pubkey(), ONE_SAFE * 2)
        .await
        .unwrap();
    let (acc, deposit_tx) = deposit(&mut context, &ahkey, &ah, &test_metadata, &buyer, ONE_SAFE);
    ...
    assert_eq!(escrow.lamports, ONE_SAFE + rent_exempt_min);
}
